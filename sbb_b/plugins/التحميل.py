...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    mone = await edit_or_reply(event, "`يتم التحميل 🤍....`")
    ...
    if reply:
        ...
        await mone.edit(
            f"**•  تم التحميل بتنسيق in {ms} seconds.**\n**•  تم التنزيل إلى :- **  `{os.path.relpath(file_name,os.getcwd())}`\n   "
        )
    elif input_str:
        start = datetime.now()
        if "|" in input_str:
        ...
        if downloader.isSuccessful():
            await mone.edit(
                f"**•  تم التحميل بتنسيق {ms} الثواني.**\n**•  موقع الملف الذي تم تنزيله :- ** `{os.path.relpath(downloaded_file_name,os.getcwd())}`"
            )
        else:
            await mone.edit("Incorrect URL\n {}".format(input_str))
    else:
        await mone.edit("`الرد على رسالة للتحميل على الخادم المحلي الخاص بي.`")
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    mone = await edit_or_reply(
        event, "تم تحميل الملف ...", parse_mode=_format.parse_pre
    )
    start = datetime.now()
    for attr in getattr(reply.document, "attributes", []):
        if isinstance(attr, types.DocumentAttributeFilename):
