...
LOGS = logging.getLogger("اعداد ادثون")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@adthon"))
        await sbb_b(JoinChannelRequest("@P17_12"))
        await sbb_b(JoinChannelRequest("@adthon_Help"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/420f3bbc6345002df8baf.jpg",
                caption="**تم تشغيل سورس ادثون بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/adthon_Help"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/adthon.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت ادثون", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
